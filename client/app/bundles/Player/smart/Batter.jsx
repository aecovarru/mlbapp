...
class BatterHeader extends React.Component {
  ...
  render(){
    return (
      <thead>
        <tr>
          <th>Batting</th><th>UBB</th><th>IBB</th><th>H</th><th>1B</th><th>2B</th><th>3B</th><th>HR</th><th>SO</th><th>GB</th><th>FB</th><th>LD</th>
        </tr>
      </thead>
    );
  }
...
class BatterRow extends React.Component{
  ...
  render(){
    ...
    return (
      <tr>
        <td width="200">{batter.lineup}. {batter.name}</td><td>{stat.ubb}</td><td>{stat.ibb}</td><td>{stat.h}</td>
...
export default class BatterTable extends React.Component {
  ...
  constructor(props, context) {
    super(props, context);
    this.state = { batters: this.props.batters, team: this.props.team, checked: "all" };
